...
function MenuView({ data }: { data: any }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4">
        <h4 className="text-xs font-black text-slate-700 uppercase tracking-wider mb-1">Dish Performance Chart</h4>
        <p className="text-[10px] text-slate-400 mb-3">Click on any quadrant below to view explanation and operational strategies.</p>
        <div className="grid grid-cols-2 gap-3">
          {[
            { key: 'star', label: 'Best Sellers', title: '🌟 Stars', desc: 'High profit + High sales — Promote actively. These are your high-profit, high-volume champions. They define your brand and drive the bulk of your revenue. Keep quality consistent and use them to anchor your marketing.', color: 'bg-emerald-100 text-emerald-800 border-emerald-200' },
            { key: 'plowhorse', label: 'Popular but Low Profit', title: '🐎 Plowhorses', desc: 'Low profit + High sales — Consider repricing upward. High volume but low margins. Customers love them, but they aren\'t making you much money. Strategy: Gently increase price or reduce portion cost.', color: 'bg-amber-100 text-amber-800 border-amber-200' },
            { key: 'puzzle', label: 'High Profit, Low Sales', title: '🧩 Puzzles', desc: 'High profit + Low sales — Market more aggressively. High margins but low demand. These are hidden gems. Strategy: Give them more visibility on the digital menu or offer a limited-time promo to trigger trial.', color: 'bg-blue-100 text-blue-800 border-blue-200' },
            { key: 'dog', label: 'Low Profit, Low Sales', title: '🐕 Dogs', desc: 'Low profit + Low sales — Consider removing or reworking. Low margins and low demand. They occupy kitchen space and mental energy without returning value. Strategy: Remove from menu or completely rework the recipe.', color: 'bg-rose-100 text-rose-800 border-rose-200' },
          ].map(m => (
            <div
              key={m.key}
              onClick={() => setActiveMatrixPopup(m.key)}
              className={`${m.color} border rounded-2xl p-3 cursor-pointer hover:scale-[1.02] active:scale-[0.98] transition shadow-xs`}
            >
              ...
              <span className="text-[9px] opacity-70 block mt-0.5">{m.desc.split('—')[0]}</span>
            </div>
          ))}
        </div>
      </div>
